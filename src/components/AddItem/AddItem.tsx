            ...
            <form className="grid grid-cols-3 gap-3 " onSubmit={handleFormSubmit}>
                {colDefs && colDefs.map((val: any) => {
                    if (!(val.field.toLowerCase() === "seq") && !(val.field.toLowerCase() === "id")) {
                        let type = val.field.toLowerCase() == "price" || val.field.toLowerCase() == "cost" ? 'number':'text'
                        console.log(type)
                        return (
                            <div key={val.field} className="flex flex-col">
                                ...
                                <input type={type} className="mt-1 p-2 w-full border border-gray-300 rounded-md focus:ring-2 focus:ring-orange-500 focus:outline-none" id={val.field} onChange={onInputChange} name={val.field} placeholder={val.field} value={formData[val.field] || ""} />
                            </div>
                        )
                    }
                })}
